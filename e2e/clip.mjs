...
// 헤드리스에선 capture(같은 단일 유틸)로 검증한다. 멱등 — 시작·끝에 clear.
//
// 사용: SOKSAK_SOCKET=~/.soksak/com.soksak.dev.sock node e2e/clip.mjs   (이 repo 루트)
// 멀티윈도우: SOKSAK_WINDOW=<프로젝트 창 label> 이면 모든 호출을 그 창으로 라우팅한다(미설정 시 앱 기본 대상).
// 종료코드: 0 = PASS, 1 = FAIL.
...
const WINDOW = process.env.SOKSAK_WINDOW || undefined;
...
function rpc(method, params = {}, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const msg = { id, method, params, ...opts };
    if (WINDOW && msg.window === undefined) msg.window = WINDOW;
...
async function main() {
  ...
  await m("clear", {}); // 전체 비우기(멱등 시작)

  // ── 클립보드 캡처/dedup/검색 ──
  section("클립보드 — 캡처·dedup·검색");
  const cap = await m("capture", { text: "비밀번호123" });
  ok(cap.ok && typeof cap.itemId === "string", "capture → itemId");
  const cap2 = await m("capture", { text: "비밀번호123" });
  ok(cap2.ok && cap2.deduped === true && cap2.itemId === cap.itemId, "같은 내용 → dedup(copyCount++)");
  await m("capture", { text: "안녕하세요 클립" });
  const sr = await m("search", { query: "클립" });
  ok(sr.ok && sr.items.some((c) => c.content.includes("클립")), "CJK 검색 적중");
  const st1 = await m("state", {});
  ...
  const memos = await m("list", { kind: "memo" });
  ...
  ok((await m("category", { id: memo.itemId, category: "업무" })).category === "업무", "메모를 업무로 이동");
  const inBiz = await m("list", { category: "업무" });
  ...
  ok((await m("list", { category: "회사" })).items.some((c) => c.id === memo.itemId), "회사 카테고리로 옮겨짐");
  const del = await m("category.delete", { name: "회사" });
  ok(del.ok && del.moved === 1, "category.delete(회사) — 항목은 기본으로");
  ok((await m("list", { category: "기본" })).items.some((c) => c.id === memo.itemId), "메모가 기본으로 복귀");
  ok((await m("category.delete", { name: "기본" })).ok === false, "기본 카테고리 삭제 거부");

  // ── 보존 purge(즐겨찾기·메모 예외) ──
  section("보존 — purge(즐겨찾기·메모 예외)");
  const keep = await m("capture", { text: "즐겨찾기 클립" });
  await m("favorite", { id: keep.itemId }); // 즐겨찾기 → 보존 예외
  const before = (await m("state", {})).clips;
  ok(before >= 2, `purge 전 클립보드 ${before}건`);
  const purge = await m("purge", { olderThanMs: 0 }); // 0ms 보다 오래 = 전부(즐겨찾기 제외)
  ...
  ok((await m("list", { favorite: true })).items.some((c) => c.id === keep.itemId), "즐겨찾기 클립 잔존");

  // ── 휴지통 ──
  section("휴지통 — 삭제·복원");
  await m("delete", { id: keep.itemId });
  ok((await m("count", { trash: true })).count >= 1, "휴지통으로 이동");
  await m("restore", { id: keep.itemId });
  ok((await m("list", { favorite: true })).items.some((c) => c.id === keep.itemId), "복원됨");

  // ── teardown ──
  section("teardown");
  ok((await m("clear", {})).ok, "clear(전체 정리)");
  ok((await m("state", {})).clips === 0, "클립보드 0");
